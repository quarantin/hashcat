...
KERNEL_FQ void m33700_init (KERN_ATTR_TMPS (pbkdf2_sha256_tmp_t))
{
...
KERNEL_FQ void m33700_loop (KERN_ATTR_TMPS (pbkdf2_sha256_tmp_t))
{
  const u64 gid = get_global_id (0);
...
KERNEL_FQ void m33700_comp (KERN_ATTR_TMPS (pbkdf2_sha256_tmp_t))
{
  const u64 gid = get_global_id (0);
  ...
  aes_iv[0] = hc_swap32_S (tmps[gid].out[ 8]);
  aes_iv[1] = hc_swap32_S (tmps[gid].out[ 9]);
  aes_iv[2] = hc_swap32_S (tmps[gid].out[10]);
  aes_iv[3] = hc_swap32_S (tmps[gid].out[11]);
  ...
  // The plaintext of first block should be `{"wallet":{test`
  u32 plaintext[4] = {
    0x7b227761,  // {"wa
    0x6c6c6574,  // llet
    0x223a7b22,  // ":{"
  ...
  aes256_encrypt_cbc (aes_ks, aes_iv, plaintext, out, s_te0, s_te1, s_te2, s_te3, s_te4);

  const u32 r0 = hc_swap32_S(out[0]);
  const u32 r1 = hc_swap32_S(out[1]);
  const u32 r2 = hc_swap32_S(out[2]);
  const u32 r3 = hc_swap32_S(out[3]);
